// 액션 타입을 선언합니다
// 뒤에 as const를 붙여줍으로써 나중에 액션 객체를 만들때 action.type의 값을 추론하는 과정에서
...
export const increaseBy = (diff: number) => ({
  type: INCREASE_BY,
  // 액션에 부가적으로 필요한 값을 paylaod란 이름으로 통일합니다.
  // 이는 FSA(https://github.com/redux-utilities/flux-standard-action) 라는 규칙인데
  // 이 규칙을 적용하면 액션들이 모두 비슷한 구조로 이루어져 있게 되어 추후 다룰 때도 편하고
...
// ReturnType<typeof ___>는 특정 함수의 반환값을 추론해줍니다.
...
const counter = (
  ...
  switch (action.type) {
    case INCREASE: // case라고 입력하고 Ctrl + Space를 누르면 어떤 종류의 action.type들이 있는지 확인할 수 있습니다.
